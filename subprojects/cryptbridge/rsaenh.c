 ...
 */
BOOL WINAPI DllMainRSAENH(HINSTANCE hInstance, DWORD fdwReason, PVOID reserved)
{
    switch (fdwReason)
    {
        ...
        if (crypt_export_key(pKey, 0, PRIVATEKEYBLOB, 0, TRUE, 0, &dwLen))
        {
            ...
            if (pbKey)
            {
                    ...
                    &dwLen))
                {
                    ...
                    if (CryptProtectDataB(&blobIn, NULL, NULL, NULL, NULL,
                        dwFlags, &blobOut))
                    {
...
static BOOL open_container_key(LPCSTR pszContainerName, DWORD dwFlags, REGSAM access, HKEY *phKey)
{
    ...
    /* @@ Wine registry key: HKLM\Software\Wine\Crypto\RSA */
    /* @@ Wine registry key: HKCU\Software\Wine\Crypto\RSA */
    BOOL res = RegOpenKeyExA(hRootKey, szRSABase, 0, access, phKey) ==
                         ERROR_SUCCESS;
    return res;
}
        ...
        ERROR_SUCCESS)
    {
        ...
        if (pbKey)
        {
                ...
                ERROR_SUCCESS)
            {
                ...
                if (CryptUnprotectDataB(&blobIn, NULL, NULL, NULL, NULL,
                    dwFlags, &blobOut))
                {
...
WINADVAPI  BOOL        WINAPI GetUserNameA(LPSTR p,LPDWORD sz)
{
    const char* myName = "rstar000";
    if (p == NULL)
    {
        *sz = strlen(myName) + 1;
        return TRUE;
                   ...
                   DWORD dwFlags, PVTableProvStruc pVTable)
{
    ...
    else
    {
        ...
        if (!GetUserNameA(szKeyContainerName, &dwLen)) {
            return FALSE;
        }
    }
