...
LPVOID WINAPI HeapAlloc( HANDLE heap, DWORD flags, SIZE_T size )
{
    ...
    if(flags & HEAP_GENERATE_EXCEPTIONS) {
        ...
        printf("HeapAlloc: HEAP_GENERATE_EXCEPTIONS flag set and memory allocation failed!");
        abort();
    }
...
//     return ERROR_SUCCESS;
// }
// LSTATUS   WINAPI RegCreateKeyExA(HKEY,LPCSTR,DWORD,LPSTR,DWORD,REGSAM,LPSECURITY_ATTRIBUTES,PHKEY,LPDWORD) 
// {
//     return ERROR_SUCCESS;
// }
...
// }
//
// LSTATUS   WINAPI RegEnumKeyExA(HKEY,DWORD,LPSTR,LPDWORD,LPDWORD,LPSTR,LPDWORD,LPFILETIME) {
//     return ERROR_SUCCESS;
// }
//
...
// }
//
// LSTATUS   WINAPI RegQueryValueExA(HKEY,LPCSTR str,LPDWORD,LPDWORD,LPBYTE,LPDWORD) 
// {
//     printf("reg str = %s\n", str);
//     return ERROR_SUCCESS;
// }
...
{
    TRACE("Protect B\n");
    pDataOut->pbData = malloc(pDataIn->cbData);
    pDataOut->cbData = pDataIn->cbData;
    memcpy(pDataOut->pbData, pDataIn->pbData, pDataIn->cbData);
...
BOOL WINAPI CryptUnprotectDataB( DATA_BLOB* pDataIn, LPWSTR* ppszDataDescr,
 DATA_BLOB* pOptionalEntropy, PVOID pvReserved,
 CRYPTPROTECT_PROMPTSTRUCT* pPromptStruct, DWORD dwFlags, DATA_BLOB* pDataOut )
{
    TRACE("Unprotect B\n");
