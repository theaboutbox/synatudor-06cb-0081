...
#include <stdint.h>
#include <stdio.h>   /* stdout, fwrite, putchar */
#include <stdlib.h>  /* EXIT_SUCCESS */
#include <uchar.h>   /* char16_t (C11) */
#include "ntstatus.h"
...
static void print_utf16(const char16_t *s)
{
    ...
    for (size_t i = 0; s[i] != 0; ++i) {
        ...
        if (w1 >= 0xD800u && w1 <= 0xDBFFu) { /* high surrogate */
            char16_t w2 = s[i + 1];
            if (w2 >= 0xDC00u && w2 <= 0xDFFFu) { /* valid low surrogate */
                uint32_t high = (uint32_t)(w1 - 0xD800u);
                uint32_t low  = (uint32_t)(w2 - 0xDC00u);
                codepoint = 0x10000u + ((high << 10) | low);
                ++i; /* consumed the low surrogate too */
            } else {
                /* Unpaired high surrogate -> replacement character U+FFFD */
                codepoint = 0xFFFDu;
            }
        } else if (w1 >= 0xDC00u && w1 <= 0xDFFFu) {
            /* Unpaired low surrogate -> replacement character U+FFFD */
            codepoint = 0xFFFDu;
        } else {
            /* Basic Multilingual Plane code point */
            codepoint = (uint32_t)w1;
        }

        /* Encode codepoint as UTF-8 and write to stdout */
        unsigned char out[4];
        int out_len = 0;
        ...
        if (codepoint <= 0x7Fu) {
            ...
            out[0] = (unsigned char)(0xC0u | ((codepoint >> 6) & 0x1Fu));
            out[1] = (unsigned char)(0x80u | (codepoint & 0x3Fu));
            out_len = 2;
        } else if (codepoint <= 0xFFFFu) {
            out[0] = (unsigned char)(0xE0u | ((codepoint >> 12) & 0x0Fu));
            out[1] = (unsigned char)(0x80u | ((codepoint >> 6) & 0x3Fu));
            out[2] = (unsigned char)(0x80u | (codepoint & 0x3Fu));
            out_len = 3;
        } else { /* <= 0x10FFFF */
            out[0] = (unsigned char)(0xF0u | ((codepoint >> 18) & 0x07u));
        ...
        /* Write the UTF-8 bytes to stdout */
        if (out_len == 1) {
            putchar(out[0]);
        } else {
                                  ...
                                  UCHAR *secret, ULONG secretlen, ULONG flags )
{
    ...
    if (secretlen && !(hash->secret = malloc( secretlen )))
    {
        heap_free( hash );
                                       ...
                                       ULONG flags )
{
    ...
    if (!alg || alg->hdr.magic != MAGIC_ALG) {printf("invalid handle\n");return STATUS_INVALID_HANDLE;}
    if (!handle) {printf("B\n"); return STATUS_INVALID_PARAMETER;}

    if (!(key = heap_alloc_zero( sizeof(*key) ))) {printf("C");return STATUS_NO_MEMORY;}
    key->hdr.magic = MAGIC_KEY;
